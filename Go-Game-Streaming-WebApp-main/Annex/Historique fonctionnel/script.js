...
startButton.addEventListener('click', () => {
      ...
      if(!camera_opened){
        ...
        startCamera();
        camera_opened = true;
        startButton.textContent = 'Fermer la caméra';
    }
    else{
        // Arrêtez la lecture du flux vidéo dans l'élément vidéo
        videoElement.srcObject = null;

        // Masquez l'élément vidéo
        videoContainer.style.display = 'none';
    
        startButton.textContent = 'ouvrir la caméra';
        camera_opened = false;
...
async function startCamera() {
    demarrer();
    try {
        // Obtenez l'accès à la caméra USB en spécifiant { video: { deviceId: deviceId } }
        const devices = await navigator.mediaDevices.enumerateDevices();
        const usbCameras = devices.filter(device => device.kind === 'videoinput');
        if (usbCameras.length > 0) {
            const deviceId = usbCameras[0].deviceId;
            const stream = await navigator.mediaDevices.getUserMedia({ video: { deviceId: deviceId } });
            videoElement.srcObject = stream;
            videoElement.play();
        } else {
            console.error('Aucune caméra USB détectée.');
        }
    } catch (error) {
        console.error('Impossible d\'activer la caméra USB :', error);
    }}
